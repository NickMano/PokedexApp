...
extension AboutModel: AboutModelActionsProtocol {
    ...
    func update(_ damageRelations: TypeRelation) {
        let doubleDamageFrom = damageRelations.doubleDamageFrom
        let doubleDamageWithoutDuplicates = Array(Set(doubleDamageFrom))
        
        let weaknesses = doubleDamageWithoutDuplicates.compactMap { PokemonType(rawValue: $0.name) }
        
        pokedexData.weaknesses = weaknesses
    }
